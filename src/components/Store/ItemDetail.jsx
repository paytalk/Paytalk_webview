...
const ItemDetail = ({setPay,index}) =>{
    ...
    return(
        ...
        <i.ItemDetailWrap>
            ...
            <i.ItemArea>
            <i.ImgArea>
                <img src={`${BaseURL}`+DB[index].data.content_image.content_img1} alt="이미지"/>
            </i.ImgArea>
            <i.ItemTitle>{DB[index].data.name}</i.ItemTitle>
            <i.Cost>
                <p>{DB[index].data.price}</p>
                <p><span>{DB[index].data.price} (최대 3회)</span></p>
            </i.Cost>
            <i.BtnArea>
                <i.Btn bgColor='#FF005C'><p>찜하기</p></i.Btn>
                <i.Btn onClick={()=>setModal(!modal)} bgColor='#5C63E5'><p>구독하기</p></i.Btn>
            </i.BtnArea>
            </i.ItemArea>
            <i.ItemDetailBg visi ={modal ? "1" : "0"} display={modal ? "4" : "-1"} onClick={()=>setModal(false)}></i.ItemDetailBg>
                ...
                <i.Btn bgColor='#5C63E5' onClick={()=>{
                    setNext(true)
                    setModal(false)
                    }}><p>구독하기</p></i.Btn>
                </i.SubScribeCycle>
        </i.ItemDetailWrap>
        {next ? <PayMent title={DB[index].data.name} price={DB[index].data.price} img={`${BaseURL}`+DB[index].data.content_image.content_img1} setNext={setNext}/> : ""}
        </>
    )
}
