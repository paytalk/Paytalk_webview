...
const ExChange = () => {
    ...
    const handleFormInput = (e) => {
        ...
        setCommission((e.target.value*0.018).toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') +'원');
        setExchange((e.target.value-e.target.value*0.018).toFixed(0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') +'원')
    };
    ...
    return(
        ...
        <e.ExchangeWrap>
            ...
            <e.ExchangeCost>
                ...
                <div><p style={{color : '#FF0000'}}>수수료</p><span style={{color : '#FF0000'}}>{Commission}</span></div>
                <hr/>
                <div><p>환전금액</p><span>{Exchange}</span></div>
            </e.ExchangeCost>
            <e.Btn onClick={()=>setComplete(true)}><p>확인</p></e.Btn>
